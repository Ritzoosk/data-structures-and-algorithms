...
const addTwo = (arr) => {
  ...
  arr.forEach(function(value) {newArr.push(value + 2);
  });
  return newArr;
}
...
const containsW = (str) => {
  ...
  const outputTest = regExToTest.test(str);
...
const isNum = (input) => {
  // Solution code here...
  const regExToTest = /\d/g ;
  const outputTest = regExToTest.test(input);
...
const isCapitalized = (str) => {
  // Solution code here...
  const newArr = str.match(/[A-Z]\w+/gm);
...
const citiesAtoJ = (arr) => {
  ...
  arr.forEach(value => {
    const matchStr = value.match(/^[A-J]\w+/gm);
  
  if(matchStr){
    newArr.push(matchStr[0]);
  }
